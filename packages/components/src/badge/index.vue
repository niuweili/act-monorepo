<template>
    <div class="badge">
        ...
        <div v-if="badgeList.length" class="badge_list">
            <div v-for="(item, index) in badgeList" :key="item.url" class="badge_list_item">
                <img
                    v-if="!item.flag"
                ...
                <div :class="['badge_list_item_big', { anm: item.flag }]">
                    ...
                    <div class="badge_list_item_big_right">
                        ...
                        <p class="badge_list_item_big_right_desc">
                            <span v-if="item.details?.getStatus === 1 || !item.details?.nextLevel">
        ...
        <teleport v-if="props.objectID === 100003" to=".ducks_bgc">
            <div v-if="maskIsShow" class="badge_mask duck" @click="clickOverlay"></div>
        </teleport>
        <!-- 跳一跳 -->
        <teleport v-else-if="props.objectID === 100001" to=".jump">
            <div v-if="maskIsShow" class="badge_mask jumps" @click="clickOverlay"></div>
        </teleport>
        <!-- 幸运盲盒 -->
        <teleport v-else-if="props.objectID === 100002" to=".wish-selling-machine">
            <div v-if="maskIsShow" class="badge_mask luck" @click="clickOverlay"></div>
        </teleport>
    </div>
</template>
<script setup>
